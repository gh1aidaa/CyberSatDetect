...
def _pick_base_model_path() -> Path:
    ...
    for name in (
        "best_model_render.keras",
        "best_model_qc_filtered.keras",
        "best_model.keras",
        "final_model.keras",
    ):
        p = app_dir / name
    ...
    return app_dir / "best_model_render.keras"
...
def fine_tune(dataset_path: Union[str, Path]) -> Tuple[Path, Path, float]:
    ...
    if X.ndim != 3 or X.shape[1] != WINDOW_LEN or X.shape[2] != N_FEATURES:
        raise ValueError(
            f"Expected normal windows (N,{WINDOW_LEN},{N_FEATURES}), got {X.shape}"
        )
    ...
    if (
        X_anom.ndim != 3
        or X_anom.shape[1] != WINDOW_LEN
        or X_anom.shape[2] != N_FEATURES
    ):
        X_anom = np.zeros((0, WINDOW_LEN, N_FEATURES), dtype=np.float32)
    ...
    if not base_path.exists():
        raise FileNotFoundError(
            ...
            "Set CSD_MODEL_PATH or add best_model.keras under app/."
        )
    ...
    def train_step(batch, anom_batch):
        with tf.GradientTape() as tape:
            ...
            # Separation: prefer REAL approved anomalies if present,
            # else fallback to pseudo anomalies.
    ...
    if X_anom.shape[0] > 0:
        anom_ds = (
            tf.data.Dataset.from_tensor_slices(X_anom)
            .shuffle(min(20000, int(X_anom.shape[0])))
            .batch(BATCH_SIZE)
            .repeat()
        )
