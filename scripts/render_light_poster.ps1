...
$pptxPath = 'C:\Users\mohan\Desktop\CyberSatDetect_Poster_Academic.pptx'
...
try {
    ...
    try {
        ...
        $outPng = Join-Path $outDir 'academic_poster_page1.png'
        $present.Slides.Item(1).Export($outPng, 'png', $widthPx, $heightPx)
        Write-Host "Saved: $outPng  ($widthPx x $heightPx)"

        $pdfPath = 'C:\Users\mohan\Desktop\CyberSatDetect_Poster_Academic.pdf'
